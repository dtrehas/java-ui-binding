...
public class TreeNodeFactory {
	...
	public static ITypedTreeNode createTreeNode(ITreeNode parent, String name, Object model, BasicReflectionTreeNodeIDContext idContext) {
		...
		if (model == null) {
			...
			if (result == null) {
				if (List.class.isAssignableFrom(model.getClass())) {
					result = new ListTreeNode(parent, name, (List<?>) model, idContext);
				} else if (Set.class.isAssignableFrom(model.getClass())) {
					result = new SetTreeNode(parent, name, (Set<?>) model, idContext);
				} else if (Map.class.isAssignableFrom(model.getClass())) {
					result = new MapTreeNode(parent, name, (Map<?, ?>) model, idContext);
				} else if (model.getClass().isArray()) {
					result = new GenericArrayTreeNode(parent, name, model, idContext);
				} else {
					result = new BasicReflectionTreeNode(parent, name, model, idContext);
				}
			}
		}
		return result;
