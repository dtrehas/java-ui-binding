...
public abstract class KTableCellEditor {
	...
	/**
	 * Called only by KTable to inform the editor of changes to the displayed
	 * cell's rectangle. KTable must not call setBounds() as this will lead to
	 * an infinite redraw loop between the KTable and the open editor.
	 * 
	 * @param rect
	 *            The Rectangle of the cell the editor is currently displaying.
	 */
	final void setCellRect(Rectangle rect) {
		// check: did the cell rectangle change? only then do we change the
		// control's bounds
		// otherwise, we will get an infinite redraw loop between the KTable and
		// the open editor
	...
	protected void onTraverse(TraverseEvent e) {
		...
		case SWT.ARROW_LEFT: {
			close(true);
			// select cell to the left; no cell merging can be in the way here,
		...
		case SWT.ARROW_RIGHT: {
			...
			/*
			 * Select cell to the right; however, the immediately adjacent cell
			 * might actually still be a part of this cell through cell merging.
			 * In that case, we will have to keep on couting up the row number
			 * until we find a cell that is not merged.
			 */
		...
		case SWT.ARROW_UP: {
			close(true);
			// select cell to the top; no cell merging can be in the way here,
			// so no special handling is necessary
