...
public abstract class KTableSortedModel extends KTableDefaultModel {
	 ...
	 * type <code>method(int col, int row)</code>) return the content ordered in
	 * the given direction.
	 * <p>
	...
	public int mapRowIndexToModel(int row) {
		...
		if (mappedRow >= getRowCount() || mappedRow < 0) {
			...
			// throw new
			// IllegalArgumentException("The model has changed, making the sort mapping invalid.\nPerform a resort after model changes!");
		}
		return mappedRow;
	}
	 ...
	 * Maps between the KTable row index and the model row index!
	 * <p>
	 * Calls the method <code>doGetCellRenderer(int, int)</code> to retrieve the
	 * cell renderer for a cell.
	 * 
	 ...
	 *             if the <code>doBeongsTo()</code> method returns a row or col
	 *             index that is larger than that given. This is a restriction
	 *             for valid cell spanning.
	 */
