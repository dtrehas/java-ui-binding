...
public class KTable extends Canvas {
	...
	protected ArrayList<KTableClickInterceptionListener> cellClickListeners;
	...
	/*
	 * this number of rows is examined in addition to the visible area when
	 * optimizing column width; counts both upwards and downwards.
	 */
	...
	public KTable(Composite parent, int style) {
		...
		cellClickListeners = new ArrayList<KTableClickInterceptionListener>(10);
	...
	protected void doCalculations() {
		...
		/*
		 * If all columns or all rows are fully visible, we must make sure that
		 * the table is scrolled completely to the left and top. Otherwise, it
		 * can happen that the user scrolls to the right / bottom, then
		 * increases window size which leads to the scrollbars being deactivated
		...
		if (sb != null) {
			if (m_Model.getColumnCount() <= getFixedColumnCount()
				...
				/*
				 * Always set minimum to 0 to avoid scrollbar bug on Windows XP
				 * where the scrollbar would be scrolled to the right despite
	...
	public Rectangle getCellRect(int col, int row) {
		...
		// throw new
		// IllegalArgumentException("The cell bounds of an overlapped, " +
	...
	/**
	 * Calculates the extra <i>visible</i> width of the cell due to cell
	 * merging, both in pixels and in number of columns.
	 * 
	 * @param col
	 ...
	 *            If <code>true</code>, returns the width in pixels. If
	 *            <code>false</code>, returns the result in number of columns.
	 * @return The width, unit depending on the parameter
	...
	private int calculateExtraCellWidth(int col, int row, boolean resultInPixels) {
		...
		// we determine the width of the cell both in pixels and in number of
		// columns
		...
		for (i = 1; col + i < m_Model.getColumnCount()
			 ...
			 * across this border and we start scrolling
			 */
		...
		/*
		 * if we did stop counting at the border of the fixed cell block above,
		 * we must now continue the count at the first visible cell after the
		 * break. this takes into account that a middle part of the merged cell
		 * is not visible anymore because the user has scrolled, while the first
		 * and last parts are still visible (the first part is visible because
		 * it is in the fixed cell block, while the last part is still visible
		...
		/*
		 * another special case: if a merged cell is only partially visible
		 * (because of scrolling), we first determine the theoretical width of
		 * the cell, disregarding that some parts are not visible. therefore, we
		 * must now subtract the width of all those columns that are not visible
		 * anymore because the user has already scrolled away.
		 */
	...
	/**
	 * Calculates the extra <i>visible</i> height of the cell due to cell
	 * merging, both in pixels and in number of rows.
	 * 
	 * @param col
	 *            The column index of the cell.
	 * @param row
	 *            The row index of the cell.
	 * @param resultInPixels
	 *            If <code>true</code>, returns the height in pixels. If
	 *            <code>false</code>, returns the result in number of rows.
	...
	private int calculateExtraCellHeight(int col, int row,
		...
		// we determine the height of the cell both in pixels and in number of
		// rows
		...
		/*
		 * another special case: if a merged cell is only partially visible
		 * (because of scrolling), we first determine the theoretical height of
		 * the cell, disregarding that some parts are not visible. therefore, we
		 * must now subtract the height of all those rows that are not visible
		 * anymore because the user has already scrolled away.
		 */
	...
	/**
	 * Calculates the number of extra columns this cell occupies due to cell
	 * merging. For example, if the cell does not span any columns, this method
	 * will return 0. If the cell spans across two columns, it will return one.
	 * However, if parts of the cell are not visible (for example, because of a
	 * combination of fixed columns and scrolling), then these parts are not
	 * counted here. This way, the caller can determine where the boundary of a
	 * cell is, taking into account the current scrolling position.
	 * 
	 * @param col
	 *            The column index.
	 * @param row
	 *            The row index.
	 * @return The number of extra columns this cell occupies due to cell
	 *         merging.
	 */
	...
	/**
	 * Calculates the number of extra rows this cell occupies due to cell
	 * merging. For example, if the cell does not span any rows, this method
	 * will return 0. If the cell spans across two rows, it will return one.
	 * However, if parts of the cell are not visible (for example, because of a
	 * combination of fixed rows and scrolling), then these parts are not
	 * counted here. This way, the caller can determine where the boundary of a
	 * cell is, taking into account the current scrolling position.
	 * 
	 * @param col
	...
	/**
	 * Calculates the area (in terms of columns and rows) that a certain cell
	 * occupies due to cell spanning. The return value is a {@link Rectangle},
	 * where {@link Rectangle#x} is the column index of the upper-left corner of
	 * the cell, {@link Rectangle#y} is the row index of the upper-left corner
	 * of the cell, {@link Rectangle#width} is the width of the cell (in number
	 * of columns) and {@link Rectangle#height} is the height of the cell (in
	 * number of rows). The width and height of a cell are always at least one
	 * if the cell is visible. A cell that does not span any other cells has
	 * height and width equal to 1.
	 * 
	 * @param col
	 *            The column index.
	 * @param row
	...
	private void drawBorderLines(GC gc, Rectangle clientArea) {
		if (m_Model.getRowCount() > 0) {
			...
			// and if we're scrolled all the way to the right, also draw right
			// border
			if (m_LeftColumn + m_ColumnsVisible == m_Model.getColumnCount()) {
...
/**
	 ...
	 *            <li><b>SWT.FULL_SELECTION</b> - Sets the
	 *            "Full Selection Mode". In the "Full Selection Mode", the table
	 *            always selects a complete row. Otherwise, each individual cell
	 *            can be selected. This mode can be combined with the
	 *            "Multi Selection Mode".</li>
	...
	/**
	 * Sets the cursor provider. If no cursor provider was specified or the
	 * specified cursor provider is <code>null</code>, the default cursor will
	 * be displayed. If a cursor provider was specified, however, it will be
	 * asked to provide a cursor for the current mouse hover position every time
	 * the mouse is moved.
	 * 
	 * @param cursorProvider
	 *            The cursor provider to set.
	 */
	...
	/**
	 * Sets the number of rows that are visible in the preferred size of the
	 * KTable (as calculated by {@link #computeSize(int, int, boolean)}).
	 * 
	...
	/**
	 * Sets the number of columns that are visible in the preferred size of the
	 * KTable (as calculated by {@link #computeSize(int, int, boolean)}).
	 * 
	...
	/**
	 * @return The row height that is used for calculating the preferred size
	 *         when a certain row should be visible but is currently not filled
	 *         with data.
	 */
	 ...
	 *            The row height that is used for calculating the preferred size
